...
#    path = r"\\10.240.142.97\02_Dashboards\02_Dashboard_Seguimiento\Matmo_Normalizaciones\Normalizacion\Normalizacion_%s.txt"%(Year_Int+Mes_Int)
    ...
    # -------------------------------------------------------------------------
##    Aflo_Temp_copy = copy.deepcopy(Aflo_Temp);
#    # Actualizando campo <NORMALIZACION_2> - Cruce por Número de OS y Periodo de OS (<NUM_OS> y <PERIODO>)
#    # Agregando campo <NORMALIZACION_2> e inicializando en NO normalizados
#    Aflo_Temp['NORMALIZACION_2'] = 'NO';
#    os_years = ['201612','201712','201812','201905']; #Ahorita para el ejercicio va a ser 2201905, pero en realidad toca 201906
#    for i in os_years:
#        # Consultando archivo de Normalizacion del periodo de intervención
#        path = r"\\10.240.142.97\Dashboards\Dashboards\02_Dashboard_Seguimiento\Matmo_Normalizaciones\Normalizacion\Normalizacion_%s.txt"%(1);
#        Norm_N2 = pd.read_csv(path, encoding='CP1252', sep='\t', decimal=',', usecols=['NIS_RAD','NUM_OS','UUCC_AGRUP2','PERIODO'], low_memory=False);
#        # Extaryendo tipo de ordenes que se consideran "Normalizadas"
#        cond = (Norm_N2['UUCC_AGRUP2'] == 'Normalización') | (Norm_N2['UUCC_AGRUP2'] == 'Adecuaciones') | (Norm_N2['UUCC_AGRUP2'] == 'Reubicaciones');
#        Norm_2 = Norm_N2.loc[cond].reset_index(drop=True);
#        # Eliminando duplicados por número de orden y periodo de orden
...
#        # Cruzando con el universo principal para detectar cuáles de estas ordenes fueron "Normalizadas"
